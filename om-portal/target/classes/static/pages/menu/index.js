...
function statusRenderer(data, type, row) {
	...
	html='<div class="custom-control custom-switch">'+
		 '   <input type="checkbox"'+checked+' onclick="disableMenu(\''+row.id+'\',\''+row.status+'\')" class="custom-control-input" id="'+id+'">'+
...
function operateRenderer(data, type, row) {
	var buttons = [];
	buttons.push('<a class="btn btn-primary btn-xs" href="#" onclick="dtOptions.lookup(\'' + data + '\', \'pages/menu/form.html\',\'查看页\',60,70)"><i class="nav-icon fas fa-folder"></i>查看</a>');
	buttons.push('<a class="btn btn-info btn-xs" href="#" onclick="openEntityWin(true, \'' + data + '\')"><i class="nav-icon fas fa-edit"></i>修改</a>');
	buttons.push('<a class="btn btn-danger btn-xs" href="#" onclick=deleteSelf("/api/menu/delete","'+data+'","dataGrid")><i class="nav-icon far fa-trash-alt"></i>删除</a>');
...
var openEntityWin = function(update, entityId, updatebtn) {
	...
	top.layer.open({
		...
		area : [ '60%', '70%' ],
		shade : [ 0.3 ],
		content : 'pages/menu/form.html?isUpdate=' + update + '&entityId=' + entityId + paramStr + '&v=' + version,
		btn : ['保存', '取消' ],
